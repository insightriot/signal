...
**Latest: v0.1.12 (2026-07-27) — M5.E9, the linear-mode & phase-ledger close-out.** Ran **ahead of M5.E8** (D-M5E9-2; IDs are identity, so E9-before-E8 is the rule working, not drift). Closed `B41`–`B45`. **All four originating bugs were reported from OUTSIDE Signal's own use** — a live `/sig:ship` on a project that has never used Epics — not found by its 1623-test suite. **`B42` (P1): a project with no Epics could not run `/sig:ship` at all**, live since v0.1.3 across nine releases, invisible because Signal-on-Signal has been Epic-mode since M4.5.E11. The FR1 retro gate is now **Epic-only** (D-M5E9-1) — a scope, not a bypass; D-E9-3 is unchanged for Epics. **`[BREAKING]`** `.planning/STATE.md` `completed_phases` is now an **append-only log of the current run** that **trims** rather than collapsing: linear projects relocate at ship to `STATE-HISTORY.md`, Epic projects archive on roll **before** the reset. Three rules that were load-bearing and unwritten are now in `references/state-schema.md`. New `completePhase()` finally records a SHIP date — impossible before, because SHIP is terminal. `B41`: the four middle phase commands now transition **at entry**, and `plan.md`'s unsatisfiable *"verify current phase is PLAN"* precondition is corrected. 1623 → **1652 tests**, proof-of-fail by baseline replay (**16 of 22 new tests fail on the pre-Epic code**). REVIEW **PASS-WITH-FIXES** — **1 Critical, in this Epic's own work**: the new `/sig:sweep` check told *every healthy project* its history had been destroyed; **removed rather than re-tuned**, because a past collapse is **not detectable from the file** (the evidence was destroyed by the bug being detected). Decisions D-M5E9-1…7. **Known limit, stated: nothing proves a phase command obeys its own instruction — that is M5.E8, now next.** New bugs `B46`, `B47`. Prior: **v0.1.11** (M5.E6) + **M5.E7** (the v2 direction audit, no code — read `analysis/SIGNAL-V2-ROADMAP.md`).
