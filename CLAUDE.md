...
**Active: nothing in flight.** M5.E15 closed and shipped as v0.1.19 (2026-08-06); `B52` shipped as v0.1.20 the same day in the **fix lane** (no six phases). **Next work is the closure-gated archive command** — Epic lane, folding in `B82` — then `M5.E14`'s shippable slice. **The queue is `.planning/BACKLOG.md`, not this file** (`D-M5E18-1`): when work is filed, only BACKLOG gets edited, so read the ordering there — including *why* that order and what was excluded on evidence.
