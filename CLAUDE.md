...
**Latest: v0.1.11 (2026-07-25) — M5.E6 doc-runtime close-out**, plus **M5.E7 — the v2 direction audit (BR-8), closed 2026-07-26 (no code; a decision).** v0.1.11 shipped `/sig:sweep` (the **18th** command), the `docs/map` ship-checklist line, FR7/**B31**, and the four M5.E5 carry-overs — **B27/B28/B29/B30 are all `fixed`**, not `needs-triage`; the Sprint-3 hygiene commands (`/sig:sweep`, CLAUDE.md de-bloat, `docs/map` Stage 1) are **all shipped**. 1561 → **1623 tests**; REVIEW PASS, 0 Critical. Decisions D-M5E6-1…5. **M5.E7 then answered "what does Signal build next?"** — 45 candidates verbed (**11 builds, 16 continue, 19 abandon**), sequenced as **M5.E8–M5.E12** and landed in `.planning/BACKLOG.md` with triggers + first slices. Deliverable: **`analysis/SIGNAL-V2-ROADMAP.md`** (read this, not `MILESTONE-5.md` § candidate scope, which is superseded). **Headline: not one straight port survived.** The COMPOUND group was cut (its premise was falsified — the knowledge was in-context at the moment of the miss **in all three documented chains**; the one genuine cross-session recurrence, `B13`, is cut separately on a deterministic-content-check ground — REVIEW narrowed this from an overstated universal), `<HARD-GATE>` was cut because it **does not exist as a mechanism** (C3), gstack's `/cso` on threat-model fit, pm-skills GTM + product discovery on product fit. **The finding everything is organized around: Signal cannot detect whether its own interventions work** — no test asserts a prompt instruction was obeyed, against a measured 7-of-12 adherence and 7.12× output spread on byte-identical code. Hence M5.E8 (measurement) is unconditional-next and gates the prompt-shaped ports. Decisions D-M5E7-1…12. **REVIEW (repurposed per AC7.4 to attack the reasoning, not code) returned PASS-WITH-FIXES** — 0 Critical, 3 Important + 1 Suggestion, all fixed in-phase: §2.1's falsification claim re-scoped from a universal, two of §6.1's three self-defences withdrawn/relabelled, and the roadmap's falsifier given a **dated check (2026-10-26)** under M5.E9 because a null result cannot fire a trigger. **14 open bugs** (`B32`–`B36` `needs-triage`, `B37`–`B45` `confirmed`) are homed in **M5.E9**. **`B42`–`B45` were cataloged 2026-07-26 from a live `nextpass` ship report** and re-scoped the slice: **`B42` is the ledger's first `P1`** — a project in **linear mode** (`current_epic: null`) cannot run `/sig:ship` at all, because the FR1 retro gate hard-halts on `NO_CURRENT_EPIC` with no bypass; live since **v0.1.3**, invisible because Signal-on-Signal has been Epic-mode since M4.5.E11, and **asserted by its own tests**, so the fix rescopes M4.5.E9's AC1-extended. `B43`/`B44`/`B45` are three distinct defects stacked in **`state.js:388-395`** (`transitionPhase` records the phase being *left*, so a SHIP date is unrecordable; set semantics applied to log data collapse a multi-unit `completed_phases` silently; existing entries are never validated, so junk becomes a permanent phantom phase) — **not linear-mode-only**: Epic mode is spared only because `setCurrentEpic` zeroes the list on every roll. Slice order: `B42`, then `B43`+`B44`+`B45` as one commit **before or with `B41`** (four phase commands never call `transitionPhase`, and `markFresh` then stamps the stale position fresh — wiring them up first would multiply the collapse sites), then `B36` and `B39`. Prior: **v0.1.10** (M5.E5), **v0.1.9** (M5.E4).
