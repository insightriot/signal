...
**Active: nothing in flight, and Milestone 6 is not opened.** `M5.E10` closed and shipped as **v0.1.25** (2026-08-13), which **closed Milestone 5** (`D-BR0809-2`). Brett's 2026-08-06 ordering is fully discharged (`B52` → v0.1.20, `/sig:archive` → v0.1.22, `M5.E14`'s slice → v0.1.24), and the command-namespace question closed 2026-08-09 — bare verbs stay, `migrate-memory` → `migrate` at the next breaking window (`D-BR0809-3`). **Next work is a decision, not a queue item, and it is Brett's.** Open and deliberately deferred: **`B99`** (an install ships ~11 MB of tracked files, 4.7 MB of it `.planning/`, against ~1.9 MB the commands need — the fix needs research, not a patch), **whether `.planning/` should be public at all** (keeping it *in the repo* was chosen; *copying it into every user's plugin cache* never was), and **the semantic claims-audit backstop** (`AC0.1`). **The queue is `.planning/BACKLOG.md`, not this file** (`D-M5E18-1`): read the candidates there, including what was excluded on evidence.
