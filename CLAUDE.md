...
**Latest: v0.1.22 (2026-08-07) — `M5.E19`, "file finished work away."** Signal could already archive closed units; it had **no command whose job that was** — archiving happened inside `/sig:migrate-memory`, a command about document *layout*, whose dry-run interleaves a reorganization plan with an archive plan. `/sig:archive` gives it a front door: closure is a **gate** (a unit not provably closed is not moved), dry-run by default. **The release's actual content is the report.** Pointed at a corpus, the pre-existing helpers named the units that *would* move and the one that *could not be evaluated* — and said **nothing** about the units considered and **refused**: three of six vanished, the stub-retrospective veto among them, which is the most surprising thing the tool does and was the least explained. Every refusal now carries the reason its resolver produced, and *"checked and clean"* never renders as *"could not check."* **The Epic's premise was false, and running it is what proved so:** `BACKLOG.md` said M5.E18 *"built the engine and wired none of it"* — a quote of that retro's **mid-Epic** finding, which its own wave 6 fixed; the word *"wired"* hid the difference between **called** and **load-bearing** (`resolveClosures` is called, inside a `try/catch`, feeding a dry-run explanation). A **safety bug** was found by the Epic's own acceptance test: the report took its plan from the half that cannot see an unreadable `STATE.md` and its refusals from the half that can, so a project with no STATE would have been handed a plan to archive work that might be the **current unit** — the retired tool's exact failure, inside its replacement. Also shipped: **`references/command-taxonomy.md`** (the five command groups that already existed, written down, with a naming rule) and **`examples/sandbox/`** (a throwaway corpus, so no command need ever be pointed at a real project — **production repos are not test beds**). Filed **`B87`**: `B41`'s shape survives in **human-driven** runs — EXECUTE ran and never entered `completed_phases` — caught while it was happening rather than reconstructed. 2282 → **2300 tests**, **20 commands**.

**Prior: v0.1.21 (2026-08-07) — `B82`, fix lane.** A closed unit archived **half of itself**, and the split was invisible. `planArchiveMoves` rebuilt every candidate from a `{unit}-{suffix}` template — a **second implementation** of *"which files belong to this unit"* that cannot express `deriveUnits`' conservative fold — so it moved what it could name and left the rest, splitting a unit across `.planning/` and `.planning/archive/`. Measured through the live command path across 12 local projects: **3 split units / 6 stranded files → 0 / 0.** **Signal's own tree could never have caught it:** every unit here is a strict Epic ID whose files really are `{EpicID}-{SUFFIX}.md`, so template and derivation agree *by construction* — 21 units, 88 moves, 0 stranded, before and after. The bug lives only where unit names are **not** Epic IDs, which is 8 of 12 real projects, so **dogfooding was structurally blind to it**. `AC6.1` held and caught the first attempt at the fix. Carried `B84`'s fix, whose first real cut this was.

**Prior: v0.1.20 (2026-08-06) — `B52`, "which copy of Signal is actually running."** Claude Code resolves a plugin's install path **once, at session start**, and holds it for the life of the **process** — so a session alive across an auto-update keeps running the version it started with while every config file on disk correctly records the new one. Three sightings in six days: one **silently discarded M5.E8's six-phase ledger** (the archive-before-reset guard existed and was correct — it just was not the code that ran), one **re-issued an instruction a release had deleted**. `tools/lib/plugin-binding.js` compares the copy the process *actually resolved* — derived from the module's own path, **never `CLAUDE_PLUGIN_ROOT`**, because the env var states where the plugin is *supposed* to be and this bug is exactly that disagreement — against what `installed_plugins.json` records. **Wired at two surfaces, and the second is not optional:** the SessionStart hook **structurally cannot see** a mid-session update (the binding is resolved before it runs), so `/sig:status` and `/sig:resume` re-read both files **at the moment of use**. **Half 2 proved wider than the bug report:** `setCurrentEpic`'s reset is unconditional, so "archive first" was a step that could be *skipped* rather than an invariant that *held* — **two** of the three branches that zero an unarchived phase log need no stale cache at all (a linear project opening its first Epic; a non-strict `current_epic` such as `PHASE11`). A log with no safe destination now **throws before any write**. Two defects found by the release's own tests: **a symlinked `HOME` would have silenced the whole check permanently** (every unit test passed while the hook printed nothing — `B39`'s shape), and **`B84`** — `cut-release.js`'s no-release-notes guard is unreachable and relabelled a historical CHANGELOG section instead of refusing. 2233 → **2269 tests**.

**Earlier: v0.1.19 (2026-08-06) — M5.E15, "the control arm, made real" (`B55`).** The adherence harness deleted the measured instruction from **one** command file while four others still ordered the same call, so the arm labelled *"instruction deleted"* contained it four more times and no run could distinguish *"the instruction works"* from *"the agent found it elsewhere."* The canary now declares all five directive sites and the control arm deletes every one; an independent leak walk verifies the tree **without consulting `canary.deletions`** (a check that reads the deletion list can only find sites the list already knows). `B41-phase-entry` re-ran **`OBEYED`** — treatment 3/3, control 0/3, seam PASS — the first verdict the harness has produced that means what it says. Also closed `B80`, `B83`.
...
**Active: nothing in flight.** `M5.E19` closed and shipped as **v0.1.22** (2026-08-07); `B82` shipped as **v0.1.21** the same week in the **fix lane**. **Next work is `M5.E14`'s shippable slice** — the `discharged` marker + a SHIP-gate open-obligations query — with **`B87`** (the phase ledger misses phases in hand-driven runs) and the **command-namespace decision** (`BACKLOG.md` item 4) both filed behind it. **The queue is `.planning/BACKLOG.md`, not this file** (`D-M5E18-1`): read the ordering there, including *why* that order and what was excluded on evidence.
