...
**Before any Workflow step**, call `await transitionPhase(baseDir, 'VERIFY')` (`tools/lib/state.js`). It appends the phase being **left** to `completed_phases` and sets `phase: VERIFY`.
