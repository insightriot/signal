...
**Before any Workflow step**, call `await transitionPhase(baseDir, 'REVIEW')` (`tools/lib/state.js`). It appends the phase being **left** to `completed_phases` and sets `phase: REVIEW`.
