...
3. **Jev (optional, off unless you set a key)** — with `TYPESAFE_API_KEY` set, Signal asks TypeSafe's Jev model whether paragraphs of your `STATE.md` contradict the project's current facts (phase, current Epic, version). It sends each paragraph with those facts to `api.typesafe.ai`. Get a key from [TypeSafe](https://typesafe.ai). No key, no call.
