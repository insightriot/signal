...
- **`/sig:migrate-memory`** — auto-sensing memory/doc migration for a bloated or old-layout `.planning/`. **Relocate-never-delete, dry-run by default, git-reversible, idempotent.** Senses frontmatter-prose, an oversized `STATE.md` body, and closed-Epic scaffolding, and (on explicit confirm) reorganizes — including the v2→v3 doc-layout transition (capture-inbox rename, `BACKLOG.md` create, closed-milestone `DECISIONS.md` eviction behind resolvable anchors) — then stamps `docs_layout_version`. An out-of-date project is prompted to run it by a fail-open banner on `/sig:resume` / `/sig:status` / SessionStart. Not tier-gated.
