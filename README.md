...
npm test                        # the full suite should pass
```
...
3. **Jev (optional, off unless you set a key)** — with `TYPESAFE_API_KEY` set — in your environment or in the project's `.env` — `/sig:resume` and `/sig:ship` ask TypeSafe's Jev model whether paragraphs of your `STATE.md` contradict the project's current facts (phase, current Epic, in-flight work, version). It sends each paragraph with those facts to `api.typesafe.ai`. Get a key from [TypeSafe](https://typesafe.ai); no key, no call. What it is and is not: **advice only** — it never blocks a ship; it reads **`STATE.md` only**; it judges only against the facts Signal derives (it cannot know what you did not record, and dates and counts are compared by code, not by the model); and it is a model's judgment, so **results can vary between runs**. `/sig:docs-sweep` never calls it.
...
- **`/sig:advise`** — read-only roadmap advisor. Reads the `.planning/` corpus — `BACKLOG.md`, `BUGS.md`, STATE/closure, milestone rows — and writes one dated advisory, `.planning/BACKLOG-REVIEW-YYYY-MM-DD.md`, naming what to work on next **and every live row it looked at and passed over, with the reason.** Every claim carries a `path:line` citation that is resolved against disk before the file is written; a citation that does not resolve fails the run and no file appears. It **proposes and never selects** — nothing is struck, nothing is queued, no other file is touched. A source it could not read is named in the artifact, never silently treated as empty. Not tier-gated.
