...
- **Review findings — read from the PR, never assumed absent.** Call `readPrReviewFindings({owner, repo, pr, execFn})` (`tools/lib/pr-review-findings.js`) and render `formatPrReviewFindings(result)`. It lists every **unresolved** review thread with file, line and headline; `cannot-check` (no `gh`, no auth, no network, no PR) renders as its own line and **never as "none"**. *Signal has run an independent reviewer on every PR since `v0.1.31` and nothing ever read its output. Measured 2026-08-23: **7 findings across 11 PRs, 7 of 7 real**, four of them merged over unread — including two holes in a guard that had a green 2886-test suite and four mutation tests behind it, and three more in the fix for those two.* ⚠ **This reports; it does not refuse** — a skipped read is process, and process warns (`B75`, 2026-08-22). ⚠ **An unresolved thread marked OUTDATED is counted and named separately, never treated as handled**: a later push marks threads outdated whether or not anything was fixed.
- [ ] All CI checks pass
