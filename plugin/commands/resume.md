...
1e. **STATE-vs-world drift** (M5.E16, FR3 / `D-M5E16-2`; extended by `M6.E2`) — call `runDriftChecks(baseDir, ALL_DRIFT_CHECKS)`, taking `runDriftChecks` from `tools/lib/state-drift.js` and **`ALL_DRIFT_CHECKS` from `tools/lib/published-facts.js`** (the composed registry — STATE drift plus the published-fact checks; passing no second argument runs only the STATE half), and pass the result to `renderResumeBriefing` as `stateDriftResult`. Read-only, offline, deterministic; fail-open (any failure → pass `null` and no line renders).
