...
describe('canary registry (AC3.1–AC3.3)', () => {
  ...
  it('every canary declares its trace BEFORE any run (AC3.3)', () => {
    ...
    for (const c of reg.canaries) {
      ...
      expect(c.deleteLine, `${c.id} must name the exact line the control arm deletes`).toBeTruthy();
    }
  });
  ...
  it("the canary's deleteLine actually exists in the command file it names", () => {
    // A registry entry whose target line has drifted out of the command file
    // would delete nothing, make both arms identical, and read as INERT.
    for (const c of loadCanaryRegistry(ROOT).canaries) {
      ...
      expect(occurrences, `${c.id}: deleteLine occurs ${occurrences}x in commands/${c.command}.md`).toBe(1);
    }
  });
});
