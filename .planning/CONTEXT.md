...
**M5.E2 — Auto-sensing migrate command (FR6/FR7) — DISCUSS complete → PLAN next (FULL/strict).** **Sequencing pivot 2026-07-16:** E2 and E3 swapped. Live doc-bloat is blocking Brett across ~5 projects (`nextpass/.planning/STATE.md` is write-wedged at 529 KB — `BUGS.md` B8), so the **migrate command** (the thing that reorganizes an *existing* bloated project's docs to the FR1 model) is pulled forward to M5.E2; the all-docs hygiene runtime + living `BACKLOG.md` (FR4/FR5) moves to M5.E3 (its 3 DISCUSS pre-decisions locked — `MILESTONE-5.md`). **Scope (D-M5E2-1…5):** full FR6/FR7 — auto-sensing, dry-run-default, git-reversible, **relocate-never-delete** with the §5 faithfulness gate, all three bloat vectors, conservative auto-sense; FR7 `docs_layout_version` stamp + upgrade banner. **The finish line is a *release*:** E1+E2 (eviction + migrate) is a coherent releasable unit — "the proper upgrade number" Brett installs everywhere and runs per-project; the command operates on the **invoking** project, not Signal's repo (except the dogfood). Not gated on E3. **Headline risk (advisor):** automated vector-1 de-prose must *relocate* frontmatter prose, never drop it (the B8 hand-recipe deleted ~80 lines — fine by hand, catastrophic unattended); faithfulness is proven by a human-approved per-project dry-run diff, **not** green tests. Spec: `M5.E2-REQUIREMENTS.md`. **Also this session:** B8 discoverability mitigation shipped (`56593a2`); B9 (`setCurrentEpic` stale-phase-on-roll) logged confirmed.
