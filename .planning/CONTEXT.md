...
**▶ OPEN: M5.E9 — the linear-mode & phase-ledger close-out (v0.1.12). DISCUSS closed 2026-07-27; NEXT is `/sig:plan`.** Spec: [`M5.E9-REQUIREMENTS.md`](M5.E9-REQUIREMENTS.md) — **8 FRs**, tier **FULL inherited** (no Epic-scoped PROFILE; this is code in the state-mutation core at a P1, which the project profile already describes). Decisions **D-M5E9-1 … D-M5E9-7**.

**Why this Epic exists, and why it runs ahead of M5.E8 (D-M5E9-2):** an outside report — a live `/sig:ship` on **`nextpass`**, a project that has run in **linear mode** (`current_epic: null`) its whole life — surfaced four confirmed bugs on 2026-07-26. **`B42` is the ledger's first `P1`:** a linear-mode project **cannot run `/sig:ship` at all**, since the FR1 retro gate hard-halts on a null `current_epic` with **no bypass** — live since **v0.1.3**, across nine releases, and **asserted by its own tests**, so the fix rescopes M4.5.E9's AC1-extended in the open. `B43`/`B44`/`B45` are **three distinct defects stacked in `state.js:388-395`**: it records the phase being *left* (so a SHIP date is unrecordable — SHIP is terminal), it applies **set semantics to log data** (silent collapse, no diff/warning/count), and it never validates existing entries (junk becomes a permanent phantom phase). **`B44`/`B45` are not linear-mode-only** — any project past its first unit is exposed; Epic mode is spared only because `setCurrentEpic` zeroes the list on every roll. **All four were invisible because Signal-on-Signal has been Epic-mode since M4.5.E11 and structurally cannot reach them** — the M5.E7 headline finding in deterministic, file-shaped form.

**Order is a constraint, not a preference:** `B42` → the `state.js` cluster as **one commit** → **`B41`**. Wiring four more commands to `transitionPhase` before the log fix multiplies the collapse sites (D-M5E9-3). Plus **one end-to-end test of a linear-mode project with a multi-unit history** (FR8) — the blind spot itself, RED against `main`.

**The DISCUSS gate settled two things (D-M5E9-6/7):** the log now trims at `/sig:ship` in linear mode (→ `STATE-HISTORY.md`) and **archives before it resets** on an Epic roll (→ `archive/<milestone>/<epic>/STATE-NARRATIVE.md`) — because a log that never trims trades silent deletion for silent growth. The archive write goes **in the state-mutation seam, not command prose (AC5.3)**: a guarantee living in a command file is exactly the `B41` failure mode. Detection lands in **`/sig:sweep`**, repair in **`/sig:migrate-memory`** — `/sig:doctor` is **out of scope**, it diagnoses the plugin install, never the project's docs (D-M5E9-4, correcting my own earlier recommendation). **Already-collapsed history is not recoverable**; sweep warns and must not imply a fix exists.

**⏭ AFTER: M5.E8 — Measurement foundation.** Keeps its ID and its slot immediately behind v0.1.12; **still the unconditional next Epic** among the roadmap items — nothing prompt-shaped can be honestly evaluated until it exists. Open with `/sig:discuss --epic M5.E8`. First slices in [`BACKLOG.md`](BACKLOG.md) § M5.E8: **(a)** one test asserting a Signal command instruction was actually obeyed (*done-when:* a stranger deletes that instruction line, re-runs, and sees it **fail**); **(b)** the second-opinion replay against the `B19` commit. **Scope note (Brett, 2026-07-26, `2bf687e`):** M5.E8 **stays scoped to instruction-adherence** — the environment-readiness axis surfaced by [`../analysis/AGENT-EFFECTIVENESS-ALIGNMENT.md`](../analysis/AGENT-EFFECTIVENESS-ALIGNMENT.md) is blocked on a permission model (`/sig:permissions`), not on this Epic. **E9-before-E8 is the ID-as-identity rule working, not drift** — `PROJECT.md` § Vocabulary names this exact case.
