...
**⏭ NEXT: M5.E8 — Measurement foundation.** No Epic is open. **M5.E8 is the unconditional next Epic** — it is the only item in the roadmap with no trigger, because nothing prompt-shaped can be honestly evaluated until it exists. Open it with `/sig:discuss --epic M5.E8`. First slices are in [`BACKLOG.md`](BACKLOG.md) § M5.E8: **(a)** one test asserting a Signal command instruction was actually obeyed (*done-when:* a stranger deletes that instruction line, re-runs, and sees it **fail**); **(b)** the second-opinion replay against the `B19` commit. **Scope note (Brett, 2026-07-26, `2bf687e`):** M5.E8 **stays scoped to instruction-adherence** — the environment-readiness axis surfaced by [`../analysis/AGENT-EFFECTIVENESS-ALIGNMENT.md`](../analysis/AGENT-EFFECTIVENESS-ALIGNMENT.md) is blocked on a permission model (`/sig:permissions`), not on this Epic. **M5.E9 can run in parallel** — it has no trigger either.
