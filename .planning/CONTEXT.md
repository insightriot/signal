...
- **`B82` is fixed by making the closure record carry its files, not by passing `deriveUnits` in alongside.** `resolveClosures` already computes `unitFiles` and discards it; `planArchiveMoves` consumes that instead of rebuilding `{unit}-{suffix}.md`. One source of truth — passing it separately leaves two callers free to disagree again, which is the defect. **Re-measured across all 12 local projects: 3 split units / 6 stranded files across `nextpass` + `cm-mentor-coach` — the two projects archiving by hand — where BUGS.md records one unit on one project. Signal's own tree shows 0, so dogfooding alone would have shipped it.** (2026-08-07) → `D-M5E19-4`
