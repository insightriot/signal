...
**M5.E7 — "the v2 direction audit" (BR-8) — 🔵 DISCUSS complete 2026-07-25; PLAN next.** Opened via `/sig:discuss --epic M5.E7` (human label: *the v2 direction audit*). The long-deferred BR-8 landscape re-audit, **reframed before scoping**: it runs on **"what does Signal need next," not "how much of each source repo have we absorbed"** (D-M5E7-1) — the nine inspiration repos become a *menu*, and `abandon` is a first-class outcome. Deliverable is `analysis/SIGNAL-V2-ROADMAP.md` (**not** `-RUNDOWN-v2`, whose name presupposes the retired frame — D-M5E7-5); no code ships. **The evidence base changed with the frame:** the primary input is Signal's own record — **16 of 18 retros carry `## What to feed back into Signal`**, never once systematically harvested — plus BUGS/BACKLOG/ISSUES-INBOX/OPEN-QUESTIONS, with the four planned-port repos re-verified live against that demand (D-M5E7-3). Sequence is **proposed, Brett ratifies**; every candidate gets **build / abandon / continue** (D-M5E7-4). **Runs under Signal's first-ever Epic-scoped PROFILE** (`M5.E7-PROFILE.md`, D-M5E7-2): `tier: FULL` with the code-ceremony flags off and strict gates kept — "strict thinking, zero test theater." SPIKE was rejected twice on substance (it skips SHIP, which disarms the retro gate; and its retro template lacks the very feedback section this Epic harvests). Spec: **`M5.E7-REQUIREMENTS.md`** (5 FRs + ACs + doc-shaped NFRs). Known gaps recorded, not papered over: **no written tester feedback exists** (`M4.5.E5-LAUNCH-KIT.md` §3 entirely unchecked), and **4 of 17 retros are stubs** — reported as findings, deliberately not reconstructed.
...
**Current work (2026-07-25):** **Milestone 5 — the doc-runtime is DONE (E1+E2+E3 → v0.1.8; close-out E6 → v0.1.11).** Signal's memory is fully self-maintaining: `ISSUES-INBOX`→`BACKLOG`/`BUGS` capture lifecycle, auto `/sig:index`, all-docs hygiene guard, verbatim DECISIONS append-log eviction, `/sig:migrate-memory`, and now `/sig:sweep` (read-only hygiene on the *invoking* project). **No Epic open.** Next horizon: the **BR-8 v2-port re-audit**, which gates the speculative ports (gstack / pm-skills / superpowers / compound-engineering); the Sprint-4 compound Epic (OBSERVATIONS.md passive capture, D-M5E6-2) is also queued. Open a new Epic with `/sig:discuss --epic <id>`.
