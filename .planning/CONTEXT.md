...
**M5.E3 — All-docs hygiene runtime + living `BACKLOG.md` (FR4/FR5) — NEXT UP (open with `/sig:discuss --epic M5.E3`).** Its 3 DISCUSS pre-decisions are locked (`MILESTONE-5.md`). **Scope decision 2026-07-18 (Brett): folded into the release** — the doc-runtime ships as ONE 0.1.x (E1+E2+E3), not E1+E2 alone. FR4 extends auto-hygiene beyond STATE/FUTURE-IDEAS to every planning doc (append-logs like `DECISIONS.md`, milestone docs — which the E2 migrate currently *flags but protects*); FR5 = a living `BACKLOG.md` (dated-snapshot relocator). See `DECISIONS.md` 2026-07-18.

**M5.E2 — Auto-sensing migrate command (FR6/FR7) — ✅ DONE THROUGH REVIEW (PASS-WITH-FIXES, 2026-07-18); SHIP folded into the combined E1+E2+E3 release.** Full DISCUSS→REVIEW at FULL/strict in one session. Shipped `/sig:migrate-memory` (relocate-never-delete, dry-run-default, git-reversible, all 3 bloat vectors + archive-tree + FR7 stamp/hook/banner) — S1 engine (t0–t8) + S2/S3 vectors/hook/banner + S4 Signal dogfood (`.planning/` 31 archive relocations + stamp v2) + nextpass faithfulness proof (546 KB→1.3 KB, **0 words dropped**, re-confirmed post-REVIEW). **REVIEW = 3-specialist adversarial panel** → caught + fixed a **SHIP-blocking rollback gap** (2 reviewers reproduced it), a directory-symlink escape, fence-less false-success, a `readLayoutBanner` perf/DoS, and test-adequacy gaps — 5 RED-first batches (`50ad065`..`dd77ef1`), 1071→**1300 tests**. Bugs B10–B16 logged (B10–B13/B11 fixed; B14/B15/B16 ticketed). Reports `M5.E2-{VERIFICATION,REVIEW}.md`. The command operates on the **invoking** project (unwedges nextpass et al.); live per-project apply happens after the combined release via `/plugin update`. Spec `M5.E2-REQUIREMENTS.md`; decisions D-M5E2-1…5.
...
**Current work (2026-07-17):** **M5.E2 (auto-sensing migrate command, FR6/FR7) — DISCUSS + PLAN complete → EXECUTE next.** The pivot is locked (migrate command pulled forward; FR4/FR5 → E3). Next command: **`/sig:execute`** (no `--epic` needed — `current_epic: M5.E2` is set; plan approved). PLAN = 4-agent research + 8-dim plan-checker (WARN → fixes folded); **4 slices / 3 waves**, S1 alone un-sticks nextpass + auto-remediates B8. The deliverable is a *release* (E1+E2 bundled) — what reaches Brett's ~5 blocked projects. Artifacts: `M5.E2-{REQUIREMENTS,RESEARCH,PLAN,VALIDATION}.md`. B9 folds in as S1.t0.
...
*Last updated: 2026-07-16 (M5.E2 auto-sensing migrate command — DISCUSS complete, PLAN next. Sequencing pivot: migrate command pulled forward from E3; FR4/FR5 hygiene/backlog → E3. B8 discoverability mitigation shipped `56593a2`; B9 logged. Prior: M5.E1 SHIPPED unreleased; 2026-07-14 v0.1.6; 2026-07-05 M4.5.E10 → v0.1.5.)*
