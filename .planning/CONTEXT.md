...
- **15 slash commands**, **26 agents**, **21 skills**, **999 tests passing**, validator green.
...
**M5.E1 — Doc-runtime & memory hygiene — IN PROGRESS (DISCUSS complete 2026-07-16).** M5's first-built Epic, FULL/strict, opened via `/sig:discuss --epic M5.E1` (first real dogfood of the E11 `--epic` flow). **Go-big doc-runtime flagship** (Brett's call): canonical doc-model — folds in the re-audit's one gating decision (FR1) — + STATE/FUTURE-IDEAS eviction (FR2/FR3) + all-docs hygiene runtime (FR4) + living `BACKLOG.md` (FR5) + an auto-sensing, **dry-run-first** migrate command (FR6, *not* `/sig:doctor`) + a `docs_layout_version` stamp / SessionStart upgrade banner (FR7). Borrows Curator's design, **no dependency** (DECISIONS 2026-07-13). Spec: `M5.E1-REQUIREMENTS.md`; decisions D-M5E1-1…6 (DECISIONS 2026-07-16). Proposed slices: **S1** model+eviction+dogfood-on-Signal's-own-`.planning/` · **S2** all-docs hygiene+backlog · **S3** migrate command+stamp (risky piece last). **Next: PLAN.**
...
*Last updated: 2026-07-16 (M5.E1 Doc-runtime & memory hygiene — DISCUSS complete, go-big flagship scope locked; M4.5 CLOSED + M4.5.E11 Epic-native flow → v0.1.7 reconciled into Current state / Active work; 894→999 tests. Prior: 2026-07-14 v0.1.6 doc-integrity guardrail; 2026-07-05 M4.5.E10 → v0.1.5.)*
