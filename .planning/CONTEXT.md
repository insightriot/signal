...
**Current work (2026-07-18):** **M5.E3 (all-docs hygiene + living BACKLOG + append-log eviction) — DISCUSS complete → PLAN next.** Next command: **`/sig:plan`** (no `--epic` needed — `current_epic: M5.E3` is set). Doc-lifecycle model locked (D-M5E3-1…8); spec `M5.E3-REQUIREMENTS.md` (6 FRs). E3's risky piece = append-log evict-with-anchors (DECISIONS.md → history behind pointers, auto-index resolves). Prior: **M5.E2 SHIPPED 2026-07-18** (`/sig:migrate-memory`, unreleased — batched into the combined E1+E2+E3 cut). B9 fixed in M5.E2.S1.t0 (`22abe65`; ledger corrected this session).
...
*Last updated: 2026-07-18 (M5.E3 DISCUSS complete → PLAN next. Doc-lifecycle model locked D-M5E3-1…8: ISSUES-INBOX/BACKLOG/BUGS/OPEN-QUESTIONS; evict-with-anchors append-log hygiene; layout v2→v3 rollout; Curator §8 retirement. Prior: M5.E2 SHIPPED unreleased 2026-07-18 (migrate command); M5.E1 SHIPPED unreleased; 2026-07-14 v0.1.6; 2026-07-05 M4.5.E10 → v0.1.5.)*
