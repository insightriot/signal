...
- **18 slash commands**, **26 agents**, **21 skills**, **1736 tests passing**, validator green.
...
**✅ CLOSED: M5.E8 — shipped as v0.1.13 (2026-07-28).** The measurement foundation. Full DISCUSS → SHIP at FULL/strict. Retro: [`M5.E8-RETROSPECTIVE.md`](M5.E8-RETROSPECTIVE.md).

**The two numbers that are the deliverable:**
- **The ceiling:** of **407 directive lines** across 18 `commands/*.md`, **91 (22.4%) are trace-measurable; 316 (77.6%) are not** — published in [`ADHERENCE-LOG.md`](ADHERENCE-LOG.md), and the log states the remainder is **unmeasured, not passing** (pinned by a wording test).
- **The verdict:** `B41-phase-entry` → **3/3 as-written, 0/3 deleted, OBEYED.** M5.E9's phase-entry fix works. It shipped unverified in v0.1.12 and is the first thing the harness measured.

**The sentence worth carrying forward:** *every defect this Epic found was in the measuring instrument, and every one produced a plausible-looking result rather than an error.* Four of them — an unreachable fixture (`ABSENT`), a broken seam, a control arm that deleted one line from a section restating the instruction three more times (near-miss `INERT`), and a commit captured after the run that defeated `--combine`'s pairing guard. **None would have made a reader doubt the number.**
...
**The number, now measured rather than estimated (S1, shipped):** across all 18 command files there are **407 directive lines**; **91 (22.4%) leave an observable trace, 316 (77.6%) do not.** Published in [`ADHERENCE-LOG.md`](ADHERENCE-LOG.md), computed by `tools/lib/directive-classifier.js` whose split rule is written out in full in its header. Slice 1 shipped **ahead of the harness**, deliberately, so the figure cannot be accused of being fitted to the tool. **AC5.3's clause is in the file and pinned by a wording test:** the untraceable remainder is *unmeasured, not passing*.

**PLAN's number was wrong, and the correction matters.** The pre-build estimate said 202 directives / 22 measurable / **10.9%**. Two of its claims did not survive contact with a real classifier: the estimate required directives to *begin* with an imperative verb (dropping the corpus's most common shape — a leading clause, e.g. `execute.md`'s own *"Before any Workflow step, call `transitionPhase(...)`"*), and it matched library calls by shape instead of resolving them against the 263 real `tools/lib` exports. **The load-bearing inference is now falsified:** research claimed `execute.md` / `verify.md` / `review.md` / `calibrate.md` name **zero** library calls, so *"the phases doing the most work are the least measurable."* They name **4 / 3 / 3** — including `transitionPhase`, this Epic's own canary. Only `calibrate.md` names zero. The canary set is **not** confined to the low-risk phases. Detail: the superseded-by-measurement block in [`M5.E8-RESEARCH.md`](M5.E8-RESEARCH.md); the estimate is left there verbatim because the plan was built on it.

**What did not move:** roughly three in four of Signal's instructions still leave no trace, and the prose-shaped guidance carrying most of the value — *surface ambiguity*, *don't rationalize*, *gate at product altitude* — remains entirely out of reach of this method.
