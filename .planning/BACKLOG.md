...
**The roster, measured from the tree (`references/agent-reachability.md`, regenerated by
...
⚠ **Three of them are named in command PROSE, which reads like dispatch and is not** — the exact
distinction `agent-reachability.md` was written to hold: `executor` in `discuss.md`, `execute.md` and
`plan.md`; `planner` in `discuss.md`; `security-auditor` in `init.md`. `execute.md` says *"the executor
agent handles 2-5"*, which a reader takes as a wiring claim. **Those three are the highest-priority
determinations**, because a prose mention is worse than silence: it advertises a capability the code
does not have.
