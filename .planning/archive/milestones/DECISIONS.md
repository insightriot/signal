# Archived Decisions — milestones

Closed-milestone decision sections relocated verbatim from `.planning/DECISIONS.md` by `/sig:migrate-memory` (append-log eviction). Byte-identical; every decision anchor still resolves here via `/sig:index`. History, not state — append-only.
