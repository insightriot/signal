...
| **E10 — Resume trust & capture integrity** | **✓ shipped 2026-07-05 (v0.1.5)** | 6 items → 5 slices, 13 tasks (`0c0ca54..dfc4bf7`): FR2 origin-drift (`isStaleVsOrigin`) + FR3 STATE freshness · FR1 Epic-prefix resolver · FR4 capture-pipe guards · FR5 schema-drift banner (status/resume, AD2) · FR6 hook harness + `references/hooks-api.md` + SD3 privacy fix. DISCUSS→SHIP at FULL/strict; VERIFY 31/31 ACs; REVIEW PASS-WITH-FIXES (2 agents caught the F1 crash — staleness checks threw on a schema-drifted STATE.md; 7 findings fixed). 777 → 854 tests, no new deps. See `M4.5.E10-RETROSPECTIVE.md`. |
| **E11 — Epic-native flow** | **✓ shipped 2026-07-15 (v0.1.7)** | Makes Epic mode first-class over a byte-identical linear default. 4 slices: S1 (`current_epic` write-half + `detectMode` + retro-gate hook fail-open + linear golden fixture), S2 (`artifactName` + thread into 6 commands + Epic golden fixture), S3 (`readEffectiveProfile` + `formatTierLine` + 6-command gate-read retrofit + calibrate/escalate targeting), S4 (bootstrap dogfood + e2e fixture + docs). FR1 `current_epic` write-half (`setCurrentEpic`/`deriveNextEpicId`), FR2 Epic-scoped artifact naming, FR3 per-Epic `{EpicID}-PROFILE.md` calibration, FR4 mode detection. DISCUSS→SHIP at FULL/strict; VERIFY PASS (no unmapped AC); REVIEW PASS-WITH-FIXES (2 agents, 1 Important fixed, 1 Suggestion rejected-with-reason — narrowing the read regex would break `v0.1.6-*` back-compat). 894 → 999 tests, no new deps. `references/epic-native-flow.md`. See `M4.5.E11-RETROSPECTIVE.md`. |
...
**Exit:** ✅ met (2026-07-05) — all six items shipped + tested, validator green, released as **v0.1.5**. 777 → 854 tests. One carry-over: AC6.4 (the real-session SessionStart-resume hook handshake) is a documented human smoke-check for the release, per `references/hooks-api.md`.
...
**Status:** ✓ shipped 2026-07-15 (v0.1.7). DISCUSS→SHIP at FULL/strict; VERIFY PASS, REVIEW PASS-WITH-FIXES (2 specialist agents). 894 → 999 tests. Cross-refs: `.planning/M4.5.E11-{REQUIREMENTS,RESEARCH,PLAN,VALIDATION,PROGRESS,VERIFICATION,REVIEW,RETROSPECTIVE}.md`, `references/epic-native-flow.md`, DECISIONS 2026-07-15 (D-E11-1…5).
