...
| **E6** | 🔨 **DISCUSS** (2026-07-22) | **Doc-runtime close-out — the maintenance-command half.** `/sig:sweep` (invoking-project, `.planning/`-aware, read-only report) + its checks + `docs/map` Stage-1 ship line + concurrency-lock the RMW paths + clear the four M5.E5 carry-overs (B27/B28 flag-not-abort, B29 guard, B30 retro-gate timing). **Finishes the doc-runtime so its self-maintenance is 100% locked before the re-audit (BR-8).** Spec: [`M5.E6-REQUIREMENTS.md`](M5.E6-REQUIREMENTS.md) (7 FRs); decisions D-M5E6-1…4. |

> **Re-audit now follows E1–E6 (2026-07-22).** The BR-8 "opening move" note below predates the doc-runtime override + the E4/E5 bug-squashes + the E6 close-out. The landscape re-audit is no longer M5's *first* Epic — it runs **after** M5.E6 finishes the doc-runtime — and still gates only the *speculative* v2 feature ports. Read the status table above for actual sequence.
