...
**Release prep — carry to the combined E1+E2+E3 marketplace release (D-M5E2-6):** the doc-runtime lands on `main` across E1/E2 intentionally unreleased; the release cuts after E3. Deferred to that cut, tracked here so it isn't silent drift: ~~(1) bump the **"N slash commands" count 15 → 16**~~ **← now resolved in M5.E3 FR4/S3** (the reconcile-before-guard step fixes every prose count against `roster.js` — 15→16 + re-verified 26 agents / 21 skills / test-count — *on `main`, before the tag*, so the FR4 hygiene guard can land green; see `M5.E3-PLAN.md` S3.t1); (2) add `/sig:migrate-memory` to the **README § Command reference** (also naturally lands with the FR4 reconcile); (3) bump `plugin.json` + tag + `marketplace.json` per the version rubric — **still a tag-time step** (the FR4 version-check skips `[Unreleased]`, so it stays green through the unreleased window and asserts correctness once the tag cuts); (4) fold the CHANGELOG `[Unreleased]` doc-runtime block under the cut version heading. Per-Epic SHIPs (E1, E2) deliberately did **not** touch the user-facing README/roster — but E3's FR4 makes the counts correct earlier (a guarded invariant), so only the version-bump/tag mechanics remain for the tag.
