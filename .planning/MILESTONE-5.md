...
| **E9** | ▶ **NEXT — opens 2026-07-27, ahead of E8** (D-M5E9-2) | **Bug & linear-mode close-out → v0.1.12.** Slice 1 is the release: **`B42`** (**P1** — a linear-mode project cannot run `/sig:ship` at all; the FR1 gate hard-halts on `current_epic: null` with no bypass, live since **v0.1.3**, and **asserted by its own tests**, so the fix rescopes M4.5.E9's AC1-extended), then the **`state.js:388-395` cluster as one commit** (`B43` a SHIP date is unrecordable because the function records the phase being *left* and SHIP is terminal · `B44` set semantics applied to log data collapse a multi-unit `completed_phases` silently · `B45` existing entries are never validated, so junk becomes a permanent phantom phase), then **`B41`** — *in that order*, because wiring four more commands to `transitionPhase` first multiplies the collapse sites. Plus **one end-to-end test of a linear-mode project with a multi-unit history** — the blind spot all four lived in, which Signal-on-Signal (Epic-mode since M4.5.E11) structurally cannot reach. Later slices keep the rest of the Epic: `B36`, `B39`, retro replay, EXECUTE dispatch guidance, SHIP-time ledger reconcile, the dated falsifier check (**2026-10-26**). Decisions **D-M5E9-1** (retro gate is Epic-only) + **D-M5E9-2** (this Epic precedes E8). |
| **E8** | ⏸ queued behind E9 — **still unconditional-next among the roadmap Epics** | **Instruction-adherence measurement** — the M5.E7 headline (*Signal cannot detect whether its own interventions work*) and the gate on every prompt-shaped port. Unchanged in scope by E9's jump: **E9 is a P1 unblock, not a re-aim.** Adopt the *"observable or dropped"* rubric rule from the Span alignment pass. Scope detail: [`../analysis/SIGNAL-V2-ROADMAP.md`](../analysis/SIGNAL-V2-ROADMAP.md). |
